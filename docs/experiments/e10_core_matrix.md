...
- `results/experiments/e10_capacity_360m_core/e10_core_execution_20260813.json`,
  the execution and integrity evidence for this report.
...
- Training-set size has a large and consistent effect at 360M. Both scale
  ...
  well clear of zero.
- Pretraining has no measurable effect at either scale. Both point estimates
  are slightly negative, both intervals contain zero, and the three seeds
  disagree in sign at both scales.
- There is no evidence that pretraining's contribution grows with scale. The
  difference in differences is +0.00445 with an interval containing zero, and
  its per-seed values span −0.01582 to +0.01919, a range four times its mean.

This reproduces at 360M what E8B found at 135M, where the published pretrained
readout B3 reached 0.50929 at train_40k and 0.59610 at train_250k against the
architecture-matched random readout B2 at 0.53392 and 0.60086. Across two model
sizes the random control matches or slightly exceeds the pretrained readout.
The comparison with E8B is context from an already-published record, not part
of the frozen E10 contrast set, and no new statistic was computed for it.
...
Projected against actual. The Gate-1 projection budgeted 49.72766 GPU-hours of
core execution. Adding the known extra development pass described below brings
the honest projection to 50.48153. Actual core execution was 51.07838, which is
2.72 per cent above the unadjusted projection, 1.18 per cent above the adjusted
one, and 82.2 per cent of the 62.19228 hour stress scenario. Per identity the
overruns were +0.69944 and +0.65129 hours.
...
1.35072 hour total overrun; the remaining 0.59685 hours is ordinary
cell-to-cell variation of about 1.2 per cent. No resource constant was altered
to absorb it, and it had no wall or ceiling consequence.
...
Two integrity observations should go to the reviewer.
...
with the real grant present. After the authorised matrix completed,
`phase1-verify` still passes in full, but `phase2-verify` and, through it,
`phase3-verify` now fail. The cause is two clauses inside
`phase2.pipeline_contract()` that assert the pre-execution state rather than
the contract: `assert_analysis_refuses_incomplete()`, which requires the real
analysis to refuse and therefore cannot survive a complete matrix, and
`e10.assert_no_scientific_cells()`, which requires zero scientific artefacts to
exist. Every other clause of all three phases still passes, checked
individually: the frozen recipe digests, the training control flow, stage
coverage, the frozen execution plan, the resource policy, the no-automatic-retry
scan, the grant schema, the proof that no project source can write a grant, the
proof that the grant lies outside every bound digest, the entry gate, all three
binding amendments, and the model verification.

Four test modules fail for the same reason. `test_e10_phase1`'s
`test_scientific_core_still_refuses` asserts that no core authorisation grant is
present; `test_e10_phase3` calls `assert_no_core_authorization_grant`, which the
Phase-3 report already documents as ceasing to hold once the user creates the
grant; `test_e10`'s wrong-wall refusal test reads the real grant instead of a
redirected one and so sees a different refusal message; and
`test_e10_phase2`'s determinism test mirrors the real governance tree into its
temporary directory and then picks up one of the twelve real
`cell_completed_*.json` records with `next(governance.glob("cell_*.json"))`
instead of its own terminated record.

None of this affects the executed science. The source and config digests are
byte-identical to the reviewed ones before the grant, after the grant and after
all twelve cells; the frozen E8A, E8B and E9 result trees are unchanged; all
twelve recipe digests, the pinned manifests, the token stores and the model
verification still match. The correct repair is to make these assertions
execution-aware, exactly as the Phase-3 repair made the refusal invariant
grant-aware, and that repair is deliberately not made here: this packet
executes a frozen protocol and does not modify reviewed source. It is recorded
for the reviewer to direct.
