...
+0.06601 with interval [0.05395, 0.07747], which excludes zero. This
comparison is capacity-confounded, because the multimodal head has a larger
input and more trainable parameters, so on its own it shows only that adding the
frozen image representation is associated with higher accuracy. Whether the
heads depend on the correct image is tested directly in
Section~\ref{sec:ch4-reliance}. Fusion, which also includes product and
absolute-difference features, reached 0.53840. The capacity controls below
determine how much of that further increase can be attributed to the explicit interactions.
...
Figure~\ref{fig:ch4-ladder} shows the complete low-data ladder. The
interaction-term ablation, reproduced in the appendix, adds supporting detail.
% XREF-PENDING: appendix figure VE1-FIG-A1
At a matched 576k budget,
...
\renewenvironment{longtable}[1]{\begin{tabular}{|p{0.20\textwidth}|p{0.17\textwidth}|p{0.075\textwidth}|p{0.20\textwidth}|p{0.095\textwidth}|p{0.075\textwidth}|}}{\end{tabular}}
\let\endhead\relax\renewcommand{\_}{\textunderscore\allowbreak}
...
In the visual-reliance experiment (v2\_06), the image input changed while each
question remained unchanged. Image embeddings were either permuted across
...
seed-mean difference of +0.0037 at 250k (seed SD of the five paired gaps
0.0044). Seeds 2 and 42 reversed its direction at 250k. No image-clustered
interval is available for the scaling study (v2\_07), and the figure's
...
\begingroup\scriptsize\setlength{\tabcolsep}{0.8pt}
\renewenvironment{longtable}[1]{\begin{tabular}{|p{0.08\textwidth}|p{0.10\textwidth}|p{0.105\textwidth}|p{0.13\textwidth}|p{0.18\textwidth}|p{0.08\textwidth}|p{0.16\textwidth}|}}{\end{tabular}}
\let\endhead\relax\renewcommand{\_}{\textunderscore\allowbreak}
...
More labelled questions improve every tested head in the stored seed means,
although the fusion advantage narrows at 250k without a clustered interval.
The paired 40k encoder comparison supports representation quality as an
important but confounded factor; at 250k SigLIP fusion is descriptively comparable to the larger
